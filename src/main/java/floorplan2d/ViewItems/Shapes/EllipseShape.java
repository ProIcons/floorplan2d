 ...
 * @author Tasos
 */
public class EllipseShape extends ItemShape {

    private Ellipse2D.Double originalShape;
    private Ellipse2D.Double translatedShape;
    
    public EllipseShape(Ellipse2D.Double shape, Item bItem, BindPoint bp) {
        this(new Dimension2D.Double(shape.getWidth(), shape.getHeight()),new Point2D.Double(shape.x, shape.y),bItem,bp);
    }

    public EllipseShape(Dimension2D.Double dim, Point2D.Double pos, Item bItem, BindPoint bp) {
        super(dim, pos, bItem, bp);
        originalShape = new Ellipse2D.Double(pos.x, pos.y, dim.width, dim.height);
        setShapeType(Type.Ellipse);
        updateShape();
    }
    ...
    public void updateShape() {
        ...
        translatedShape = new Ellipse2D.Double(x1, y1, width, height);
